...
var compile = exports.compile = module.exports.compile = function(input, options) {
  ...
  return compileResult(fnResult);
}

function compileResult(partialResult, options, results) {
...
function buildCode(node, options, depth, index, code) {
  ...
  function _push(result) {
    _code(resultFnName + '.push({result:' + result + ',depth: ' + depth + ',index: ' + index + '});');
  }
  ...
  if (node.code && node.code.trim().length > 0) {
    ...
    if (node.type == '-') {
      ...
      _code('__stack_' + depth + '.push(' + node.code);
  ...
  if (node.children && node.children.length > 0) {
    for (var i = 0; i < node.children.length; i++) {
      ...
      if (i == node.children.length - 1) {
        _code('if(__stack_' + (depth + 1) + '.length>0){');
        _code('return Promise.all(__stack_'+(depth+1)+').then(function(result){})');
        _code('}');
      }
    }
  }
...
compile('<div><% var temp="traier"; %><%- \'Ion\' %><%= "Vasile" %></div><%+ getUsers().then(function(result){ %><p><%= result %></p><% }) %><p><%= temp %></p></body>', {
  locals: {
    getUsers: function() {
