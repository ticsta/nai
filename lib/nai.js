...
function buildCode(node, options, depth, index, code, parent, temp, aNode) {
  var resultFnName = options.resultFnName;
  var stack = aNode && options.stackName + '.' + aNode.id;
  temp = temp || [];
  aNode = aNode || node;
  ...
  if (node.children && node.children.length > 0) {
    ...
    for (var i = 0; i < node.children.length; i++) {
      ...
      buildCode(child, options, depth + 1, i, code, node, temp, node.isAsync ? node : aNode);
    }
    if (node.isAsync)
