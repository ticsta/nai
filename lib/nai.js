...
function buildCode(parent, options, code, openedNodes, asyncOpenedNodes) {
	...
	parent.childs.forEach(function(node) {
		if (node.isText()) {
			code.push(options.rootNodeName + '.list["' + node.id + '"].pushResult();');
			return;
		}
		if (node.isAsync) {
			code.push(options.rootNodeName + '.list["' + node.id + '"].pushResult(' + node.code);
		...
		//js = js.replace(/\\'/g, '\'');
		if (node.method == '=') {
			code.push(options.rootNodeName + '.list["' + node.id + '"].pushResult(escape(' + js + '));');
		} else if (node.method == '-')
			code.push(options.rootNodeName + '.list["' + node.id + '"].pushResult(' + js + ');');
		else {
			var oNode;
			if (node.isClosing) {
