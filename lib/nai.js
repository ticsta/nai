...
function buildCode(parent, options, resultNode, code, openedNodes) {
	code = code || [];
	...
	parent.childs.forEach(function(node) {
		...
		if (node.isAsync) {
			...
			return buildCode(node, options, newResultNode, code, openedNodes);
		}
		...
		if (node.method == '=') {
			...
			if (node.childs.length > 0)
				buildCode(node, options, resultNode, code, openedNodes);
		}
	});
